...
void HarmCurrent::DrawDC(wxPoint2DDouble translation, double scale, wxDC& dc) const
{
	...
	if (m_inserted) {
		wxPoint arrowPts[2];
		wxPoint2DDouble p;
		arrowPts[0] = RotateAround(wxPoint2DDouble(m_position.m_x, m_position.m_y - m_height / 2.0), m_position, m_angle);
		arrowPts[1] = RotateAround(wxPoint2DDouble(m_position.m_x, m_position.m_y - m_height / 2.0) + wxPoint2DDouble(0, 40), m_position, m_angle);
		...
		if (m_selected) {
			...
			// Draw node selection.
			DrawDCCircle(pointListInt[0], 5.0 + m_borderSize / scale, dc);
		}
