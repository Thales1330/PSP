	...
	: SimulationsSettingsFormBase(parent)
{
	...
	m_radioBoxBusFreqEstimation->SetSelection(static_cast<int>(data.busFreqEstimation));
	m_checkBoxBusFreqIgnoreEvent->SetValue(data.ignoreBusFreqEventStep);
	m_textCtrlTf->SetValue(Element::StringFromDouble(data.tf));
	m_textCtrlTw->SetValue(Element::StringFromDouble(data.tw));
...
bool SimulationsSettingsForm::ValidateData()
{
	...
	data.busFreqEstimation = static_cast<BusFreqEstimation>(m_radioBoxBusFreqEstimation->GetSelection());

	if (m_checkBoxBusFreqIgnoreEvent->IsShown()) {
		data.ignoreBusFreqEventStep = m_checkBoxBusFreqIgnoreEvent->GetValue();
	}

	if (m_textCtrlTf->IsShown()) {
		if (!Element::DoubleFromString(
			this, m_textCtrlTf->GetValue(), data.tf,
			_("Value entered incorrectly in the field \"Time constant Tf (Bus Frequency)\".")))
			return false;
	}
	if (m_textCtrlTw->IsShown()) {
		if (!Element::DoubleFromString(
			this, m_textCtrlTw->GetValue(), data.tw,
			_("Value entered incorrectly in the field \"Time constant Tw (Bus Frequency)\".")))
			return false;
	}
...
void SimulationsSettingsForm::UpdateBusFreqFieldStatus()
{
	bool visible = true;
	if (m_radioBoxBusFreqEstimation->GetSelection() == 0) visible = false;
	m_checkBoxBusFreqIgnoreEvent->Show(!visible);
	m_staticTextTf->Show(visible);
	m_textCtrlTf->Show(visible);
	m_staticTextTw->Show(visible);
	m_textCtrlTw->Show(visible);
	m_staticBitmapDiagram->Show(visible);
}

void SimulationsSettingsForm::OnBusFreqEstimationSelect(wxCommandEvent& event)
{
	UpdateBusFreqFieldStatus();
}
