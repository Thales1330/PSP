	...
	std::vector< std::shared_ptr<ConnectionLine> >& ctrlConnectionList)
{
	...
	// auto elementsNode = XMLParser::AppendNode(doc, rootNode, "ControlElements");
	auto ctrlElementContainer = new ControlElementContainer();
	if (!OpenControlElements(doc, elementsNode, ctrlElementContainer)) return false;

	ctrlElementList.clear();
	ctrlConnectionList.clear();
	auto elementRawList = ctrlElementContainer->GetControlElementsList();
	for (auto& element : elementRawList) {
		ctrlElementList.emplace_back(element);
	}
	auto connectionRawList = ctrlElementContainer->GetConnectionLineList();
	for (auto& connection : connectionRawList) {
		ctrlConnectionList.emplace_back(connection);
	}
	...
	ControlElementContainer* ctrlContainer)
{
	...
	if (constListNode) {
		...
		while (constNode) {
			...
			elementList.emplace_back(constant);
	...
	if (expListNode) {
		...
		while (expNode) {
			...
			elementList.emplace_back(exponential);
	...
	if (limiterListNode) {
		...
		while (limiterNode) {
			...
			elementList.emplace_back(limiter);
	...
	if (multiplierListNode) {
		...
		while (multiplierNode) {
			...
			elementList.emplace_back(multiplier);
	...
	if (dividerListNode) {
		...
		while (dividerNode) {
			...
			elementList.emplace_back(divider);
	...
	if (mathListNode) {
		...
		while (mathExprNode) {
			...
			elementList.emplace_back(mathExpr);
	...
	if (connectionListNode) {
		...
		while (connNode) {
			...
			cLine->UpdatePoints();
			connectionList.emplace_back(cLine);
