...
class LineCD : public DrawObj
{
private :
	//disable copying
	LineCD(const LineCD&rhs);
	LineCD&operator=(const LineCD&rhs);

	LineInternal *ptr;

public :
	LineCD(LineInternal *_ptr) : DrawObj(Line2DrawObj(_ptr)), ptr(_ptr) {}
	~LineCD() {}
...
public :
	...
	LineCD*addLine(int x1, int y1, int x2, int y2, int color = Chart::LineColor, int lineWidth = 1)
	{ LineCD*ret = new LineCD(CBaseChart_addLine(ptr, x1, y1, x2, y2, color, lineWidth)); reg(ret); return ret; }
